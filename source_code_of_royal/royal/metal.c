...
#define syntax6 \
" bilateral-filter blur-radius=4 edge-preservation=9  bilateral-filter blur-radius=1 edge-preservation=3 mean-curvature-blur iterations=2   "\

#define syntax7 \
" id=dv gimp:layer-mode layer-mode=normal opacity=0 aux=[ ref=dv  emboss depth=3 elevation=25  id=divcall ] unsharp-mask scale=0.3 mean-curvature-blur iterations=2 id=1 gimp:layer-mode layer-mode=hsl-color aux=[ ref=1 color-overlay value=#ffbb00 ]  alien-map cpn-1-frequency=6 cpn-2-frequency=6 cpn-3-frequency=1 saturation scale=0 bloom strength=95 levels in-high=1.87  in-low=0.02 noise-reduction iterations=5 id=color  gimp:layer-mode layer-mode=hsl-color opacity=0.84 composite-mode=clip-to-backdrop aux=[ color value=#f4cd62] unsharp-mask scale=0.9 median-blur  abyss-policy=none radius=0  gimp:layer-mode layer-mode=divide opacity=0.03 aux=[ ref=divcall gaussian-blur  clip-extent=false  abyss-policy=none std-dev-x=0.5 std-dev-y=0.7 noise-reduction ] "\
